...
export default function MenuLanding() {
  ...
  const theme = useMemo(() => ({ primary: p1 || '#ef4444', secondary: p2 || '#b91c1c' }), [p1, p2]);
  const onOpen = () => { if (url) window.open(url.startsWith('http') ? url : `https://${url}`, '_blank'); };
    ...
    <div className="min-h-screen w-full" style={bodyStyle}>
      <div className="w-full" style={{ background: `linear-gradient(135deg, ${theme.primary}, ${theme.secondary})` }}>
        <div className="max-w-3xl mx-auto px-5 pt-8 pb-10 text-white">
          ...
          {url && (
            <a href={url.startsWith('http') ? url : `https://${url}`} target="_blank" rel="noreferrer" className="inline-block mt-3 underline decoration-white/60 underline-offset-4 opacity-95 hover:opacity-100">
              {url}
            </a>
          )}
        </div>
