...
import { Globe, Youtube, Instagram, Cloud, Music2 } from "lucide-react";
import { useRouter } from "next/router";

export default function Mp3Landing() {
  const { query } = useRouter();
  const url = (query.url || "").toString();
  const title = (query.title || "MP3").toString();
  const desc = (query.desc || "").toString();
  const site = (query.site || "").toString();
  const cta = (query.cta || "Play").toString();
  ...
  const SocialIcon = ({ name }) => {
    ...
    if (k === 'soundcloud') return <Cloud className={cls} />;
    if (k === 'tiktok') return <Music2 className={cls} />;
    if (k === 'spotify') return <Music2 className={cls} />;
      ...
      <div className="min-h-screen w-full" style={bodyStyle}>
        <div className="max-w-2xl mx-auto px-5 py-10">
          ...
          <div className="mt-6 rounded-xl border border-black/10 bg-white overflow-hidden p-4">
            {url ? (
              unlock ? <audio controls className="w-full" src={url} /> : (
              ...
              <div className="p-6 flex items-center justify-center h-[120px] bg-gray-50">
                <div className="text-center">
                  <div className="text-4xl mb-2">🎵</div>
                  <div className="text-sm text-black/60">Upload or provide an MP3 URL to preview.</div>
                </div>
              </div>
            )}
          </div>
          ...
          {!!socials.length && (
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2">
              {socials.map((s, i) => (
                <a key={i} href={s.url || '#'} target="_blank" rel="noreferrer" className="text-sm px-3 py-2 rounded border border-black/10 bg-white hover:bg-black/5 inline-flex items-center gap-2">
                  <SocialIcon name={s.platform} />
                  <span>{s.text || s.platform || 'Link'}</span>
                </a>
              ))}
            </div>
          )}
          ...
          <div className="max-w-5xl mx-auto px-5 pt-10 pb-14 text-white">
            ...
            <div className="mt-8 bg-white rounded-xl shadow-sm border border-white/20 overflow-hidden p-4">
              {url ? (
                ...
                <div className="p-6 flex items-center justify-center h-[140px] bg-white/80">
                  <div className="text-center text-black">
                    <div className="text-4xl mb-2">🎵</div>
                    <div className="text-sm opacity-80">Upload or provide an MP3 URL to preview.</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {!!socials.length && (
              <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2">
                {socials.map((s, i) => (
                  <a key={i} href={s.url || '#'} target="_blank" rel="noreferrer" className="text-sm px-3 py-2 rounded border border-white/30 bg-white/10 hover:bg-white/20 text-white inline-flex items-center gap-2">
                    <SocialIcon name={s.platform} />
                    <span>{s.text || s.platform || 'Link'}</span>
                  </a>
                ))}
        ...
        <div className="grid md:grid-cols-5 gap-4">
          <div className="md:col-span-3 bg-white rounded-xl shadow-sm border border-black/10 overflow-hidden p-0">
            ...
            {url ? (
              ...
              <div className="p-6 flex items-center justify-center h-[140px] bg-gray-50">
                <div className="text-center">
                  <div className="text-4xl mb-2">🎵</div>
                  <div className="text-sm text-black/60">Upload or provide an MP3 URL to preview.</div>
                </div>
