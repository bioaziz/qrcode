...
export default function SmsLanding() {
  ...
  const title = (query.title || "Send an SMS").toString();
  ...
  const cta = (query.cta || "Open SMS").toString();
  ...
  const onSms = () => {
    ...
    window.location.href = `sms:${to}${qs}`;
  };
    ...
    <div className="min-h-screen w-full" style={bodyStyle}>
      ...
      <div className="max-w-3xl mx-auto px-5 -mt-8 pb-10">
        <div className="bg-white rounded-xl shadow-sm border border-black/10 p-4 max-w-sm">
          <button onClick={onSms} className="px-4 py-2 rounded text-white w-full" style={{backgroundColor: theme.primary}}>{cta}</button>
        </div>
      </div>
    </div>
  );
