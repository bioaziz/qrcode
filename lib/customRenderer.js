...
function drawFinderCircle(ctx, cx, cy, module, colors, bgFill, variant = 'solid', innerType = 'dot') {
  // Standard finder pattern is 7 modules outer; defaults below mimic spec
  const rOuter = module * 3.5;
  if (variant === 'ring') {
    const ringWidthUnits = 0.6; // thinner outer ring
    const innerDotUnits = 1.2; // smaller inner mark to create visible gap
    ...
    ctx.fill();
    // Carve inner of ring to background up to rGapOuter
    ctx.beginPath();
  ...
  const rInner = module * 1.5; // inner mark ~3 modules
  // Outer filled circle (ring background)
  ctx.beginPath();
  ctx.fillStyle = colors.outer;
  ctx.arc(cx, cy, rOuter, 0, Math.PI * 2);
  ctx.fill();
  // Punch middle by overdrawing with background fill
  ctx.beginPath();
  ctx.fillStyle = bgFill;
...
export async function renderCustomQR(canvas, opts) {
  ...
  const pixels = Math.floor((area - quiet * 2) / count);
  const size = pixels * count;
  const offsetX = Math.floor((width - size) / 2) + quiet;
  const offsetY = Math.floor((height - size) / 2) + quiet;
  ...
  for (const [cx, cy] of centers) {
    drawFinderCircle(ctx, cx, cy, pixels, { outer: csColor, inner: cdColor }, bgFill, variant, innerType);
  }
  ...
  if (image) {
    ...
    if (imageOptions.hideBackgroundDots) {
      // Clear area beneath logo by overdrawing with background fill
      ctx.fillStyle = bgFill;
