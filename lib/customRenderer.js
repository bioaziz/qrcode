...
export async function renderCustomQR(canvas, options) {
    ...
    // Check if we need circular border
    const needsCircularBorder = borderOptions?.circularBorder;
    let circularRadius = 0;
    
    if (needsCircularBorder) {
        // Calculate circular border dimensions
        circularRadius = Math.min(width, height) / 2 - 10; // 10px padding from edge
        const centerX = width / 2;
        const centerY = height / 2;
        
        // Draw circular background pattern
        drawCircularPattern(ctx, centerX, centerY, circularRadius, qrSize / 2, {
            patternColor: borderOptions.patternColor || '#f0f0f0',
            moduleSize: moduleSize * 0.8
        });
        
        // Draw border text along circle
        if (borderOptions.borderText) {
            drawCircularText(ctx, centerX, centerY, circularRadius - 15, {
        ...
        if (borderOptions.borderLogo) {
            await drawBorderLogo(ctx, centerX, centerY, circularRadius, {
    ...
    if (!backgroundOptions.color || backgroundOptions.color !== 'transparent') {
        ...
        if (needsCircularBorder) {
            ...
            const centerY = height / 2;
            ctx.beginPath();
            ctx.arc(centerX, centerY, qrSize / 2 + 10, 0, 2 * Math.PI);
...
function drawCircularPattern(ctx, centerX, centerY, outerRadius, innerRadius, options) {
    ...
    // Create ring area with random QR-like modules
    const ringWidth = outerRadius - innerRadius;
    const moduleCount = Math.floor(ringWidth / moduleSize);
    
    ctx.fillStyle = patternColor;
    
    // Generate random pattern in ring area
    for (let angle = 0; angle < 360; angle += 15) {
        for (let r = innerRadius + moduleSize; r < outerRadius; r += moduleSize * 1.5) {
            if (Math.random() > 0.4) { // 60% fill rate for pattern
                const x = centerX + Math.cos(angle * Math.PI / 180) * r;
                const y = centerY + Math.sin(angle * Math.PI / 180) * r;
                
                ctx.beginPath();
                ctx.arc(x, y, moduleSize / 3, 0, 2 * Math.PI);
                ctx.fill();
            }
        }
    }
