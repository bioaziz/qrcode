...
export async function renderCustomQR(canvas, options) {
    ...
    if (needsCircularBorder) {
        ...
        // Ensure inner radius does not exceed the space available inside outer radius minus half strokes
        const maxInnerAllowed = outerRadius - (innerBorderWidth / 2) - (outerBorderWidth / 2);
        innerRadius = Math.min(innerRadius, snap(Math.max(0, maxInnerAllowed)));
    }
