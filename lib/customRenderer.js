...
export async function renderCustomQR(canvas, opts) {
  ...
  // Circular stamp border: random pattern ring, arc text, optional logo
  const cx = width / 2;
  const cy = height / 2;
  const outerR = Math.min(width, height) / 2;
  const innerR = size / 2;
  // Fill ring with random faux modules
  if (borderOptions?.patternColor) {
    ctx.fillStyle = borderOptions.patternColor;
    for (let y = 0; y < height; y += pixels) {
      for (let x = 0; x < width; x += pixels) {
        const dx = x + pixels / 2 - cx;
        const dy = y + pixels / 2 - cy;
        const dist = Math.hypot(dx, dy);
        if (dist >= innerR && dist <= outerR && Math.random() > 0.5) {
  ...
  // Draw arc text along outer circle
  if (borderOptions?.borderText) {
    ctx.save();
    const radius = outerR - pixels;
    ctx.translate(cx, cy);
    ctx.rotate(-Math.PI / 2);
    ctx.fillStyle = borderOptions.borderTextColor || "#000";
    ctx.font = borderOptions.borderFont || "16px sans-serif";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    const chars = Array.from(borderOptions.borderText);
    const total = chars.reduce((acc, ch) => acc + ctx.measureText(ch).width, 0);
    const offset = total / radius / 2;
    ctx.rotate(-offset);
  ...
  if (borderOptions?.borderLogo) {
    const ratio = Math.max(0.05, Math.min(0.3, Number(borderOptions.borderLogoSize || 0.15)));
    const logoSize = outerR * ratio;
    const angle = -Math.PI / 2; // top
    const r = outerR - logoSize / 2;
    await new Promise((resolve) => {
      const img = new Image();
      img.crossOrigin = "anonymous";
      img.onload = () => {
        const lx = cx + r * Math.cos(angle) - logoSize / 2;
        const ly = cy + r * Math.sin(angle) - logoSize / 2;
        ctx.drawImage(img, lx, ly, logoSize, logoSize);
        resolve();
      };
      img.onerror = () => resolve();
      img.src = borderOptions.borderLogo;
    });
  }
